...
export function baseEngine() {
    ...
    const setMap = (string) => {
        ...
        if (string.constructor == Object)
            throw new Error('setMap() takes a string, not a dict.'); // https://stackoverflow.com/a/51285298
    ...
    const clearText = () => { gameState.texts = []; };
    const getTile = (x, y) => {
        if (y < 0)
            return [];
        if (x < 0)
    ...
    const api = {
        ...
        setSolids,
        setPushables,
        setBackground: (type) => { gameState.background = type; },
